#!/usr/bin/env python3
"""
Generate complete Pydantic models from weaver-forge-complete.yaml
This creates all models needed for the refactored Roberts Rules + OTel system
"""
...
def generate_pydantic_models():
    """Generate all Pydantic models using Weaver"""
    
    print("🚀 Generating Complete Pydantic Models for Refactored System")
    print("=" * 60)
    
    # Create temporary registry structure
    registry_dir = "temp_complete_registry"
    ...
    subprocess.run(["cp", "weaver-forge-complete.yaml", f"{model_dir}/"], check=True)
    
    print("📋 Semantic Groups Defined:")
    print("  • agent - AI agents in distributed systems")
    print("  • otel.communication - OTel-based messaging")
    print("  • roberts.enhanced - Roberts Rules with OTel")
    print("  • motion.otel - Motions with span tracking")
    print("  • agent.file_analysis - Code analysis by agents")
    print("  • validation.concurrent - Layer validation")
    print("  • dev_team.meeting - Dev meetings with analysis")
    print("  • scrum.scale - Scrum at Scale")
    print("  • quine.validation - Semantic quine validation")
    print("  • benchmark.ollama - LLM performance metrics")
    print()
    
    # Generate with Weaver
    print("🔧 Running Weaver generation...")
    ...
    if result.returncode == 0:
        ...
        generated = list(Path("output/complete_system").rglob("*.py"))
    ...
    # Create integrated models file
    print("\n📝 Creating integrated Pydantic models...")
    create_integrated_models()
    
    return True

def create_integrated_models():
    """Create the integrated Pydantic models file"""
...
'''
    ...
    with open("complete_pydantic_models.py", "w") as f:
        f.write(models_content)
    
    print("✅ Created complete_pydantic_models.py")

def generate_usage_example():
    """Generate example usage of the models"""
...
'''
    ...
    with open("example_model_usage.py", "w") as f:
        f.write(example)
    
    print("✅ Created example_model_usage.py")

if __name__ == "__main__":
    # Generate everything
    if generate_pydantic_models():
        generate_usage_example()
        
        print("\n🎉 Complete model generation successful!")
        print("\n📚 Next steps:")
        print("1. Review complete_pydantic_models.py for all models")
        print("2. Run example_model_usage.py to see them in action")
        print("3. Use these models to refactor the entire system")
        print("4. All models support OTel span tracking natively!")